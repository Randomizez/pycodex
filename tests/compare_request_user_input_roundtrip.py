...
from pycodex.cli import build_agent, build_cli_queue, build_model
...
def main() -> "None":
    ...
    comparison = {
        ...
        "comparison": {
            "plan_mode_prompt_equal": (
                upstream_capture.collaboration_text
                == pycodex_capture.collaboration_text
            ),
            "function_call_equal": (
                upstream_capture.function_call_item
                == pycodex_capture.function_call_item
            ),
    ...
    if not comparison["comparison"]["function_call_output_equal"]:
        ...
        print(
            json.dumps(
                upstream_capture.function_call_output_item, ensure_ascii=False, indent=2
            )
        )
        print()
        ...
        print(
            json.dumps(
                pycodex_capture.function_call_output_item, ensure_ascii=False, indent=2
            )
        )
...
class ScriptedOriginServer:
    def __init__(
        self, *, model_id: "str", response_bodies: "typing.Tuple[str, ...]"
    ) -> "None":
...
def load_roundtrip_capture(label: "str", capture_root: "Path") -> "RunCapture":
    ...
    captures = [(path, json.loads(path.read_text())) for path in request_files]
    first_index = None
...
def extract_collaboration_text(
...
def extract_function_call(
    ...
    for item in request_body.get("input", []):
        ...
        if (
            ...
            and item.get("name") == "request_user_input"
        ):
            return item
    return None


def extract_function_call_output(
    request_body: "typing.Dict[str, object]",
) -> "typing.Union[typing.Dict[str, object], None]":
    for item in request_body.get("input", []):
        if not isinstance(item, dict):
